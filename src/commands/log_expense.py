...
def initialize_database(db_path):
    if not os.path.exists(db_path):
        ...
        create_expenses_table(conn)  # Ensure the expenses table is created
        conn.close()
...
class LogExpense(commands.Cog):
    ...
    async def log_expense(self, ctx, amount: float, *, description: commands.clean_content):
        """
        ...
        language = get_user_language(user_id)  # Retrieve user's preferred language
        ...
        # Try to add the expense to the database
        try:
            # Open database connection
            conn = sqlite3.connect(db_path)
            expense_id = insert_expense(conn, user_id, amount, description)

            # Generate and send confirmation message in the user's preferred language
            ...
            logging.error(f"Unexpected error when logging expense: {e}")
            await ctx.send(translate("update_failed", language, error=str(e)))
